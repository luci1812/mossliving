...
(function (FlickityFade, bodyScrollLock, MicroModal, Rellax, Flickity, themeCurrency, Sqrl, axios, themeAddresses, ellipsed, FlickitySync, AOS) {
  ...
  const selectors$R = {
    ...
    addressContent: '[data-address-content]',
  ...
  class Addresses {
    ...
    init() {
      if (this.addressNewForm) {
        ...
        const buttonCancel = section.querySelectorAll(selectors$R.btnCancel);
        const content = section.querySelector(selectors$R.addressContent);

        if (newButtons.length) {
          newButtons.forEach((element) => {
            element.addEventListener('click', function () {
              newAddressFormInner.classList.remove(selectors$R.classHide);
              content.classList.add(selectors$R.classHide);
              element.classList.add(selectors$R.classHide);
        ...
        if (buttonCancel.length) {
          buttonCancel.forEach((element) => {
            element.addEventListener('click', function () {
              newAddressFormInner.classList.add(selectors$R.classHide);
              content.classList.remove(selectors$R.classHide);              
              newButtons.forEach((elm) => {
                elm.classList.remove(selectors$R.classHide);
              });
            });
          });
        }
  ...
  class ProductAddButton {
    constructor(wrapper, isCartItem) {
      ...
      this.cartDrawer = document.querySelector(selectors$z.cartDrawer);
      if (this.button) {
    ...
    loadForm() {
      window
        ...
        .then((response) => {
          ...
          this.updateTotal();
          (function cartUpsell() {
            var flkty = new Flickity( '.cart-upsell-product', {
              pageDots: false
            });
          })();
        });
  ...
  const selectors$s = {
    ...
    buttonViewAll: '[data-button-view-all]',
    buttonTemplate: '[data-predictive-search-button-template]',
  };
  ...
  class SearchPredictive {
      ...
      fetch(serialized)
        ...
        .then((response) => {
          ...
          if (combined.length) {
            ...
            this.injectOther(results, terms);
    ...
    injectOther(results, terms) {
      ...
      let countWrap = document.querySelector('[data-product-title-wrap] .item__count');
        $.ajax({
          url: `/search?view=ajax&q=${terms}&options[prefix]=none&type=product`,
          success: function(data){        
            $('.search__products__count').html(data);

            countWrap.innerHTML = $('.search__products__count').html();     
          },
  ...
  register('cart-drawer', [productSliderSection]);
  ...
  register('testimonials', [slideshowSection]);
  ...
  class Toggle {
    constructor(toggle) {
      this.toggle = toggle;
      this.buttonLayout = document.querySelectorAll('[data-toggle-grid]');
      this.layout = document.querySelector('.js-grid');
      this.value = this.toggle.getAttribute(selectors$6.toggle);
      this.gridLarge = this.layout.getAttribute(selectors$6.large);
      this.gridSmall = this.layout.getAttribute(selectors$6.small);
      this.toggleClickFunction = (evt) => this.toggleClick(evt);

      this.init();
      window.onload = (event) => {
        this.isActiveButtonLayout();
      }
    }
    ...
    isActiveButtonLayout() {
      this.buttonLayout.forEach( button => {
        const isLarge = document.querySelector('[data-sidebar-toggle]').offsetWidth >= options.breakpoint;
        if ( this.gridLarge == button.getAttribute(selectors$6.toggle) && isLarge ) {
          button.classList.add('active');
        }

        if ( this.gridSmall == button.getAttribute(selectors$6.toggle) && !isLarge ) {
          button.classList.add('active');
        }
      });
    }
    ...
    toggleClick(evt) {      
      ...
      let parentElm = evt.target.closest('.collection__layout');
      if (parentElm.querySelector('.active') != null) {
        parentElm.querySelector('.active').classList.remove('active');
      }
